...
public class Server {
	...
	public static void sendData(EntityPlayerMP player, Object... obs) {
		try {
			Crash.Channel.sendTo(new FMLProxyPacket(getBytes(obs),"CrashMod"), player);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public static void sendAssociatedData(Entity entity, Object... obs) {
		try {
			...
			Crash.Channel.sendToAllAround(new FMLProxyPacket(getBytes(obs),"CrashMod"), point);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	public static void sendToAll(Object... obs) {
		try {
			Crash.Channel.sendToAll(new FMLProxyPacket(getBytes(obs),"CrashMod"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
